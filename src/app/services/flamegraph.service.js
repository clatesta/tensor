 ...
 */
 (function() {
     ...
     */
     function FlameGraphService($log, $rootScope, $http) {
        ...
        */
        function generate() {
            $http.get('http://' + $rootScope.properties.host + ':' + $rootScope.properties.port + '/pmapi/' + $rootScope.properties.context + '/_fetch?names=generic.systack')
                .success(function() {
                    $rootScope.addAlert('success', 'generic.systack requested!');
                    $log.info('generic.systack requested');
                }).error(function() {
                    $rootScope.addAlert('danger', 'failed requesting generic.systack!');
