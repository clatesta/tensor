 ...
 */
 ...
 (function() {
    ...
    * @desc
    */
    function PerCpuUtilizationMetricDataModel(WidgetDataModel, MetricListService, TensorService) {
                ...
                angular.forEach(cpuSysMetric.data, function(cpuSysInstance) {
                    if (cpuSysInstance.values.length > 0) {
                        ...
                        if (angular.isDefined(cpuUserInstance)) {
                            cpuSysLastValue = _.last(cpuSysInstance.values);
                            cpuUserLastValue = _.last(cpuUserInstance.values);
                            if (cpuSysLastValue.x === cpuUserLastValue.x) {
                                returnValues.push({
                                    key: cpuSysInstance.key,
                                    val: {x:cpuSysLastValue.x, y:(cpuSysLastValue.y + cpuUserLastValue.y) / 1000}
                                });
                            }
                        }
                    }
